...
router.post('/create', (req, res) => {
  ...
  if (errors) {
    ...
    mongoose.model('users').findOne({ email: req.body.email }, function (err, user) {  // for checking user email exist or not!
      if (user) {
...
router.get("/userslist", (req, res) => {
  mongoose.model('users').find({}, function (err, user) {
    if (err) {
      res.send("problem with getting data ");
    }
    else {
      //mongoose.model('users').count({name:"hari"}, function(err, count) { // getting records count based on condition
      mongoose.model('users').count({}, function (err, count) {  // getting total records count
        var jsonlist = {
          "status": "200",
...
router.put('/:id', (req, res, next) => {
  ...
  mongoose.model('users').findByIdAndUpdate(id, req.body, function (err, userupdate) {
...
router.delete('/:id', (req, res) => {
  const id = req.params.id;
  mongoose.model('users').findByIdAndRemove(id, req.body, function (err, userdelete) {
    console.log(err);
    if (err) {
      res.send("no data found belongs to id for delete");
    }
    else {
      var jsondel = {
        "status": "200",
...
router.delete('/delete/all', (req, res) => {
  //const id = req.params.id;
  mongoose.model('users').remove({}, function (err, userdelete) {
    console.log(err);
    if (err) {
